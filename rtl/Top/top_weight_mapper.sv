...
(
    ...
    logic [PIXEL_W-1:0] stage1_weights [3456];   // conv1 weight stream (needs lane reorder, see stage1_idx)
    logic [PIXEL_W-1:0] stage2_weights [3456];   // conv2 weight stream (already in physical order)
    logic [PIXEL_W-1:0] stage3_weights [3456];   // conv3 weight stream (already in physical order)
    logic [PIXEL_W-1:0] active_weights [3456];   // mux output: whichever stream src_sel selects (used for stage2/stage3 path)
