...
//     reused columns being exactly why only 48 of 144 words are
//     re-loaded for windows 1..NUM_H_WIN-1 -- see Bank Rotation notes
//     above). Computed as the AND of two independent per-axis
//     conditions, sweep_idx being the vertical analog of win_idx:
//       col axis: win_idx  ==0 -> new only at conv_col==0
//                 win_idx   >0 -> new at conv_col==0,2,4,6
...
#(
    ...
    parameter int NUM_H_WIN    = 32,   // horizontal windows per sweep (32)
    parameter int NUM_SWEEPS   = 32    // vertical sweeps per frame (32s)
)
(
    //=======================================================
    // Controls
    ...
    output wire  logic [2:0]                            state_o,        // 000=IDLE 001=LOAD 010=FETCH 011=WAIT_FETCH 100=SECOND_BEGIN_OF_SWEEP 101=END_OF_SWEEP (assign, for TB)
    output wire  logic                                  frame_start_o,     // signal to indicate the first of every new frame 
    output wire  logic                                  frame_start_stg2_3_o  // signal for indicating the start of any new frame but in stages 2 and 3 as they came later than stage 1
);
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        ...
        //===================================================
            ...
            //---------------------------------------------------
            // END_OF_SWEEP -> WAIT_FETCH: reset conv position; rotate banks AFTER load completes
            //---------------------------------------------------

            if (state == END_OF_SWEEP && next_state == WAIT_FETCH) begin
                conv_row <= '0;
                conv_col <= '0;
    ...
    // gen_lane_valid_mask
    //=======================================================
    // valid_mask_o[g]: one instance per lane, driven by `assign`. Held low
    // outside FETCH, same as the original default '0 + conditional overwrite.
    // g_lane_valid[g] already accounts for BOTH row and column overflow
    // (see gen_lane_pos above), so no additional logic is needed here.
    generate
    ...
    // slice of conv_pixels_o via a continuous `assign`.
    // Each lane g picks its row-half (RH) and col-half (CH) from its fixed
    ...
    always_ff @( posedge clk, negedge rst_n ) begin : frame_counter
        if(!rst_n) begin
            frame_counter_o <= 0;
        end else if (((win_idx == NUM_H_WIN - 1) && (sweep_idx == NUM_SWEEPS - 1)) && conv_done_o) begin
            frame_counter_o <= frame_counter_o + 1;
        end
    end
    ...
    assign frame_start_o  = (win_idx == 5'd0 && sweep_idx == 5'd0);
    assign frame_start_stg2_3_o = (win_idx == 5'd1 && sweep_idx == 5'd0);
