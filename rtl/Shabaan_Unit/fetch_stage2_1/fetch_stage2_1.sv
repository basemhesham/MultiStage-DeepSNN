...
module fetch_stage2_1 #(
    ...
    input  logic                        first_frame, // high during frame 1 - suppresses the read
    ...
    always_ff @(posedge clk or negedge arst_n) begin
        if (!arst_n) begin
            cur_bank  <= '0;
            prev_bank <= '0;
        end else if (frame_start && !first_frame) begin
            prev_bank <= cur_bank;
    ...
    assign mem_rd_en   = in_valid & ~first_frame & ~in_reg_region;
    assign mem_rd_addr = MEM_ADDR_WIDTH'({rd_bank, local_pos});

    assign reg_rd_en   = in_valid & ~first_frame & in_reg_region;
    ...
    assign mem_wr_en   = valid_d1 & ~in_reg_region_d1;
    assign mem_wr_addr = MEM_ADDR_WIDTH'({wr_bank, local_pos_d1});

    assign reg_wr_en   = valid_d1 & in_reg_region_d1;
