// =====================================================================================
// Module name : fetch (after linting)
// Purpose     : Generates the read and write addresses/enables for the LIF history
//               BRAM. Pure address/control-timing module.
//
//               ** Spare-bank ping-pong: N active banks + 1 intermediate/spare bank. **
//               Instead of double-buffering the whole memory (2x depth), this uses
//               TOTAL_BANKS physical banks (default 9) but only sweeps ACTIVE_BANKS
//               of them (default 8) in any one frame. The write window for frame N
//               and the read window for frame N+1 are the SAME 8-bank window (frame
//               N+1 reads exactly what frame N wrote) - so the write window only
//               ever needs to avoid the ONE bank the read side is currently on, which
//               is guaranteed because the write window's start bank decrements by
//               exactly one bank every frame (mod TOTAL_BANKS), continuously
//               "rotating away" from whichever bank the trailing read is still using.
//
//               Frame 1 : write window = banks 0,1,2,3,4,5,6,7 (start=0)      no read
//               Frame 2 : write window = banks 8,0,1,2,3,4,5,6 (start=8)      read window = 0,1,2,3,4,5,6,7 (start=0, = frame 1's write)
//               Frame 3 : write window = banks 7,8,0,1,2,3,4,5 (start=7)      read window = 8,0,1,2,3,4,5,6 (start=8, = frame 2's write)
//               Frame 4 : write window = banks 6,7,8,0,1,2,3,4 (start=6)      read window = 7,8,0,1,2,3,4,5 (start=7, = frame 3's write)
//               Frame N : write window start = (10 - N) mod TOTAL_BANKS       read window start = write window start of frame N-1
//               ... and so on, start bank decrementing by 1 every frame, wrapping mod TOTAL_BANKS.
//
//               Within a frame, the window always walks FORWARD through ACTIVE_BANKS
//               consecutive banks (mod TOTAL_BANKS) starting at that frame's start
//               bank - e.g. start=8 with TOTAL_BANKS=9, ACTIVE_BANKS=8 walks
//               8,0,1,2,3,4,5,6 (wrapping past 8 straight to 0).
//
//               ** ADDRESSING (no divider/modulo hardware): **
//               Each bank is BANK_DEPTH deep (must be a power of 2). The sweep
//               position is tracked directly as (bank_adv, local_pos) - local_pos
//               counts 0..BANK_DEPTH-1 and rolls over into bank_adv (0..ACTIVE_BANKS-1)
//               instead of deriving them from a flat counter via '/' and '%'. The
//               physical bank actually being accessed is (start_bank + bank_adv)
//               wrapped mod TOTAL_BANKS via a plain compare-and-subtract (no '%').
//               The final physical address is a bit-concatenation of that bank index
//               with local_pos (bank_index * BANK_DEPTH + local_pos, done as {bank,
...
module fetch #(
    ...
    parameter ACTIVE_BANKS = 8,                              // banks swept per frame (the "sweep window" size)
    parameter TOTAL_BANKS  = 9,                              // total physical banks = ACTIVE_BANKS + spare bank(s)
    ...
    always_ff @(posedge clk or negedge arst_n) begin
        if (!arst_n) begin
            ...
            prev_bank <= '0;   // unused while frame_count=1
    ...
    // entirely during frame 1 (frame_count=1), since there is no
