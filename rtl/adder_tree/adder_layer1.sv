...
Notes:
    ...
    - Default configuration supports three 18-bit signed inputs and produces
      a 20-bit signed output without overflow.
------------------------------------------------------------------------------
*/

module adder_layer1 #(
    parameter IN_WIDTH  = 18,
    parameter OUT_WIDTH = 20 // Output width = IN_WIDTH + ceil(log2(3)) to prevent overflow
)(
    input  logic signed [IN_WIDTH-1:0]  add_1,      // First 18-bit signed input operand
    input  logic signed [IN_WIDTH-1:0]  add_2,      // Second 18-bit signed input operand
    input  logic signed [IN_WIDTH-1:0]  add_3,      // Third 18-bit signed input operand
    output logic signed [OUT_WIDTH-1:0] adder_out   // 20-bit signed sum: add_1 + add_2 + add_3
);

    logic signed [47:0] P_full;       // Full 48-bit DSP48E2 output
    logic signed [47:0] add_1_ext;    // Sign-extended version of add_1
    logic signed [47:0] add_2_ext;    // Sign-extended version of add_2
    logic signed [47:0] add_3_ext;    // Sign-extended version of add_3

    // Sign-extend each 18-bit operand to the DSP48E2's 48-bit internal datapath
...
`else
    DSP48E2 #(
        ...
        .A              ({{(30-IN_WIDTH){add_1[IN_WIDTH-1]}}, add_1}), // A input is 30 bits wide of the dsp
        .D              ({{(27-IN_WIDTH){add_3[IN_WIDTH-1]}}, add_3}), // D input is 27 bits wide of the dsp
        .C              ({{(48-IN_WIDTH){add_2[IN_WIDTH-1]}}, add_2}), // C input is 48 bits wide of the dsp
        .B              (18'sd1),
        .PCIN           (48'b0),
