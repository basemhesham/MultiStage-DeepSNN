...
def load_image_text(path: str, rows: int = 256, cols: int = 256,
    ...
    if any(ch.isspace() for ch in sample):
        ...
        # RTL pixel bus is `signed [PIXEL_W-1:0]` (e.g. signed [17:0] for
        # PIXEL_W=18). For the binary case, row_width *is* the exact bit
        # width, so we can unambiguously two's-complement-decode: any value
        # whose top bit is set represents a negative fixed-point pixel and
        # must be sign-extended, not read as a large unsigned number. (The
        # hex case is left as unsigned raw -- a hex-digit count doesn't
        # unambiguously imply a bit width, e.g. 5 hex digits = 20 bits for
        # an 18-bit field, so sign-extension there would be a guess.)
        sign_bit_value = 1 << (row_width - 1) if base == 2 else None

        pixels = []
        for line in data_lines:
            for i in range(0, len(line), row_width):
                raw = int(line[i:i + row_width], base)
                if sign_bit_value is not None and raw & sign_bit_value:
                    raw -= (sign_bit_value << 1)  # two's-complement sign-extend
                pixels.append(raw)
...
def load_image(path: str, rows: int = 256, cols: int = 256,
    ...
    Detection is based on the file's actual CONTENT, not its extension:
    a file is treated as text if a sniff of its first few KB is entirely
    printable ASCII/whitespace with no NUL bytes, and as raw binary
    otherwise. This matters in practice -- e.g. `input_t0.bin` is named
    ".bin" but its content is a text header + fixed-width binary-digit
    rows, so an extension-first check would misroute it straight into
    the raw-binary reader and fail. The extension is only consulted as a
    tie-breaker for a zero-length/unreadable-sniff edge case.
