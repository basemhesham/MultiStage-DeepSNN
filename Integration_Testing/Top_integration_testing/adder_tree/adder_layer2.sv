...
Notes:
    ...
    - Default configuration supports three 20-bit signed inputs and produces
      a 22-bit signed output without overflow.
------------------------------------------------------------------------------
*/

module adder_layer2 #(
    parameter IN_WIDTH  = 20,
    parameter OUT_WIDTH = 22 // Output width = IN_WIDTH + ceil(log2(3)) to prevent overflow
)(
    input  logic signed [IN_WIDTH-1:0]  add_1,      // First 20-bit signed input operand (Layer 1 partial sum)
    input  logic signed [IN_WIDTH-1:0]  add_2,      // Second 20-bit signed input operand (Layer 1 partial sum)
    input  logic signed [IN_WIDTH-1:0]  add_3,      // Third 20-bit signed input operand (Layer 1 partial sum)
    output logic signed [OUT_WIDTH-1:0] adder_out   // 22-bit signed sum: add_1 + add_2 + add_3
);

    logic signed [47:0] P_full;      // Full 48-bit output from the DSP48E2
    logic signed [47:0] add_1_ext;   // Sign-extended version of add_1 to match the DSP 48-bit datapath
    logic signed [47:0] add_2_ext;   // Sign-extended version of add_2 to match the DSP 48-bit datapath
    logic signed [47:0] add_3_ext;   // Sign-extended version of add_3 to match the DSP 48-bit datapath

    // Sign-extend each 20-bit operand to the DSP48E2's 48-bit internal datapath
