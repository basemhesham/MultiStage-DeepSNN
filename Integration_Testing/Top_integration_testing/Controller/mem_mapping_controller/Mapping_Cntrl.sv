...
(
    ...
    logic [7:0] load_cnt;            // number of read requests issued this load phase
    logic       full_load;           // 1 = full load (144 words), 0 = partial load (48 words)
    ...
    logic [4:0] conv_row;            // conv window GROUP top-left row within the active region
    logic [4:0] conv_col;            // conv window GROUP top-left col; steps by WIN_GROUP each cycle
    ...
    logic [4:0] conv_slides_row, conv_slides_col;       // 14 or 20
    ...
    logic [4:0] row_cnt,  row_cnt_d;    // current logical row during LOAD
    ...
    logic [1:0] tgt_bank_c;      // target bank for this write
    logic [2:0] col_base_c;      // first col_in_bank index for this word
