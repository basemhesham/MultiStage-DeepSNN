...
(
    ...
    begin
        ...
        //===================================================
        if (~arst_n)
            for (int x = 0; x < PARALLEL_MACS; x++)
                acc_mac[x] <= '0;
        else if (state == ST_IDLE && start)
            for (int y = 0; y < PARALLEL_MACS; y++)
                acc_mac[y] <= '0;
        //===================================================
        // Accumulate
        //===================================================
        ...
        begin
            if (input_idx == 1)
                for (int l = 0; l < PARALLEL_MACS; l++)
                 
                    acc_mac[l] <= ACCUM_WIDTH'($signed(product[l]));  
                         
            else
                for (int b = 0; b < PARALLEL_MACS; b++)
                   
                    acc_mac[b] <= acc_mac[b] + $signed(ACCUM_WIDTH'($signed(product[b])));
        end
    end
    ...
    begin
        ...
        begin
            ...
            for (int j = 0; j < PARALLEL_MACS; j++)
            begin   
                  
                neuron <= 32'(completed_batch_base) + 32'($unsigned(j));
                bias_wide <= ACCUM_WIDTH'($signed(FC1_BIAS[neuron]));     // sign-extend bias to ACCUM_WIDTH
                bias_aligned <= bias_wide <<< FRAC_BITS;        // rescale bias to match acc_mac's scale
                total       <= acc_mac[j] + bias_aligned;      // dot-product + bias
                result <= DATA_WIDTH'(total >>> FRAC_BITS);        // rescale back down to Q_.FRAC_BITS
                if (result < 0)
                    result <= '0;                               // ReLU: clamp negative to zero
                fc_out[neuron] <= result [17: 0];
            end
        end
    end
